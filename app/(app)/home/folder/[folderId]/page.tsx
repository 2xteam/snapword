...
export default function FolderInsidePage() {
  ...
  const parentHref = folder?.parentFolderId
    ? `/home/folder/${String(folder.parentFolderId)}`
    : "/home";
  ...
  return (
    <div style={{ display: "grid", gap: "0.75rem" }}>
      {/* Header: back icon + folder name + create buttons */}
        ...
        {!loaded ? (
          ...
          items.map((it) => (
            <li key={it.id} style={{ display: "flex", alignItems: "center" }}>
              <Link
                href={it.kind === "folder" ? `/home/folder/${it.id}` : `/vocab/${it.id}`}
                style={explorerRow}
              >

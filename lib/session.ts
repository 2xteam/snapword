export type SessionUser = {
   ...
   *
   * ⚠️ 세션을 다시 저장할 때 이 값을 빠뜨리지 말 것. 위 `token` 과 같은 이유다.
...
/**
 * 이 앱에서 **쓸 수 있는** 세션인가 — 서명 토큰이 있어야 한다.
 * 서버 API 가 토큰만 믿으므로(lib/auth.ts) 토큰 없는 세션은 화면만 열리고 API 는 401 이 된다.
 * 판단 기준을 AuthGate · useSession 과 한 곳으로 모은다 → 30-Patterns/인증과 세션 공유.md
 */
